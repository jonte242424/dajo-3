 ...
 */
import { Chord, Note, Key, Scale, Interval, Progression, } from "tonal";
...
// ─── Validate ─────────────────────────────────────────────────────────────────
export function isValidChord(input) {
    if (!input || input.trim() === "")
        return false;
...
export function transposeSections(sections, semitones) {
    if (semitones === 0)
        return sections;
...
export function getChordSuggestions(partial, limit = 12) {
    if (!partial)
        return COMMON_QUALITIES.slice(0, 8).map((q) => `C${q}`);
    ...
    if (!hasRoot) {
        ...
        return ALL_ROOTS.filter((r) => r.toLowerCase().startsWith(normalized.toLowerCase())).slice(0, limit);
    }
    // Detect root length (1 or 2 chars: "C", "F#", "Bb")
    const rootLen = ["Db", "Eb", "F#", "Gb", "Ab", "Bb", "C#", "D#", "G#", "A#"].some((r) => normalized.startsWith(r)) ? 2 : 1;
    const root = normalized.slice(0, rootLen);
    const qualityPrefix = normalized.slice(rootLen).toLowerCase();
    const matchingQualities = COMMON_QUALITIES.filter((q) => q.toLowerCase().startsWith(qualityPrefix));
...
export function analyzeChordInKey(chordSymbol, keyString) {
    try {
        ...
        const chords = isMinor
            ? keyData.natural.chords
            : keyData.chords;
        const idx = chords.findIndex((c) => Chord.get(c).tonic === parsed.root);
        if (idx === -1)
            return { numeral: "?", function: "Non-diatonic", diatonic: false };
...
export function suggestChordsForKey(keyString) {
...
// ─── Scale information ────────────────────────────────────────────────────────
export function getScaleInfo(root, scaleName) {
