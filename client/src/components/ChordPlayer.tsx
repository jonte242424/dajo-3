...
async function getChordNotes(symbol: string): Promise<string[]> {
  try {
...
let samplerCache: any = null;
let samplerLoading = false;
const samplerListeners: Array<(s: any) => void> = [];

async function getSampler(): Promise<any> {
...
// ─── Knapp: spela ett ackord ──────────────────────────────────────────────────

interface ChordPlayerButtonProps {
  chords: string[]; // lista med ackordnamn att spela i sekvens
  className?: string;
}

export function ChordPlayerButton({ chords, className = "" }: ChordPlayerButtonProps) {
